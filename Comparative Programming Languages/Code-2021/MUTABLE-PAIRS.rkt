...
(define (exp->doc exp)
  (cases expression exp
    ...
    (newpair-exp (exp1 exp2)
              (let ((val1 (exp->doc exp1))
                    (val2 (exp->doc exp2)))
                (hs-append (kw "newpair") lparen (align val1) comma (align val2) rparen)))
    (left-exp (exp1)
               (let ((val1 (exp->doc exp1 )))
                 (hs-append (kw "left") lparen (align val1) rparen)))
    (right-exp (exp1)
               (let ((val1 (exp->doc exp1 )))
                 (hs-append (kw "right") lparen (align val1) rparen)))
    (setleft-exp (exp1 exp2)
              (let ((val1 (exp->doc exp1))
                    (val2 (exp->doc exp2)))
                (hs-append (kw "setleft") space (align val1) space (char #\=) space (align val2) )))
    (setright-exp (exp1 exp2)
              (let ((val1 (exp->doc exp1))
                    (val2 (exp->doc exp2)))
                (hs-append (kw "setright") space (align val1) space (char #\=) space (align val2) )))
    ))
...
(define (expval->doc v)
  (cases expval v
    ...
    (mutpair-val (p) (cases mutpair p
                       (a-pair (left-loc right-loc)
                               (h-append (kw "pair") lparen (kw "@") (text (number->string left-loc)) comma (kw "@") (text (number->string right-loc)) rparen ))))
    ))
...
(define KEYWORDS (list "in" "let" "zero?" "if" "then" "else" "proc" "letrec" "begin" "end" "set" "newpair" "setleft" "setright" "left" "right"))
...
(define exp-parser
  (<any>
   ...
   (parser-compose (token "zero?")
                   ...
                   (token ")")
                   (return (zero?-exp e)))
   (try (parser-compose (token "letrec")
   ...
   (parser-compose (v <- psymbol)
                   (return (var-exp v)))
   (try (parser-compose (token "newpair")
                        (token "(")
                        (e1 <- exp-parser)
                        (token ",")
                        (e2 <- exp-parser)
                        (token ")")
                        (return (newpair-exp e1 e2))))
   (try (parser-compose (token "left")
                        (token "(")
                        (e <- exp-parser)
                        (token ")")
                        (return (left-exp e))))
   (try (parser-compose (token "right")
                        (token "(")
                        (e <- exp-parser)
                        (token ")")
                        (return (right-exp e))))
   (try (parser-compose (token "setleft")
                        (e1 <- exp-parser)
                        (token "=")
                        (e2 <- exp-parser) 
                        (return (setleft-exp e1 e2))))
   (try (parser-compose (token "setright")
                        (e1 <- exp-parser)
                        (token "=")
                        (e2 <- exp-parser) 
                        (return (setright-exp e1 e2))))
   )
  )

(define (string->program s)
  (a-program (parse-result (parser-compose (e <- exp-parser) ws $eof (return e)) s)))

;;; Example programs
(require "MUTABLE-PAIRS-examples.rkt")
(define examples
